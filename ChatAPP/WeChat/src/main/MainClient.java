...
public class MainClient extends javax.swing.JFrame {
    ...
    public static void updateConversationFile(String sender, String receiver , String clock, String type, String fileName){
        try{
            ...
            if(Globals.getMyEmail().equals(receiver.toLowerCase())){
                ...
                myObj.createNewFile();
                fileName = AESencryption.decrypt(fileName, Globals.getClientkey());
            ...
            fileName = AESencryption.encrypt(fileName, Globals.getClientkey());
            // secret key to string
            String key = AESencryption.convertSecretKeyToString(Globals.getClientkey());
            System.out.println("encrypted message :"+fileName);
            bw.write(type+"@@@"+sender+"@@@"+clock+"@@@"+fileName+"@@@"+key+"\n");
            bw.close();
        }
        catch(Exception e){
            e.printStackTrace();
    ...
    private void init(){
        ...
        setTitle("WeChat - "+Globals.getMyEmail());
        initEvent();
    }
